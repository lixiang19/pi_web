...
export interface CoreDeps {
	...
	discoverAgents: (cwd: string) => Promise<Array<{ enabled?: boolean; mode: string; [key: string]: unknown }>>;
	createAgentSummary: (agent: unknown) => unknown;
	createAgentConfigResponse: (agent: unknown) => unknown;
	getAgentByName: (cwd: string, name: string, scope?: string) => Promise<unknown>;
	saveAgent: (cwd: string, name: string, payload: unknown, opts: unknown) => Promise<unknown>;
	deleteAgent: (cwd: string, name: string, scope: string) => Promise<string>;
	getAutomationStore: () => { listRules: () => unknown[]; createRule: (data: unknown) => unknown; updateRule: (id: string, data: unknown) => unknown | null; getRule: (id: string) => unknown | null; removeRule: (id: string) => unknown | null };
	...
	createTransientCatalogSession: (cwd: string) => Promise<{ session: unknown; resourceLoader: unknown }>;
	fileManager: { resolveManagedFileLocation: (opts: unknown) => Promise<{ rootPath: string; targetPath: string }>; listDirectoryEntries: (dir: string, root: string) => Promise<Array<{ kind: string; path: string; [key: string]: unknown }>> };
...
// ===== Workspace Tasks (JSON single-file) =====
