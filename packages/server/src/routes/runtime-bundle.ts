...
import { authenticateDeviceToken, registerDevice, type DeviceRecord } from "../devices.js";
...
export function createDeviceRegistrationRouter(deps: RuntimeBundleDeps) {
	...
	router.post("/api/devices/register", async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			await deps.getRidgeDb();
			const device = await registerDevice(parsed);
			if (!device.token) {
				throw new Error("Device token was not generated");
			}
			res.status(201).json({
