...
const emitSessionSnapshot = async (record: SessionRecord): Promise<void> => {
...
const persistSessionRecordMetadata = async (
	record: SessionRecord,
): Promise<void> => {
...
const parseStoredSessionMessageLine = (line: string): AgentMessage | null => {
...
app.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			const payload = terminalCreateSchema.parse(
				req.body ?? {},
			) as TerminalCreateRequest;
...
app.post(
	"/api/projects/:id/worktrees/validate",
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			const projectRoot = await resolveProjectRoot(String(req.params.id));
