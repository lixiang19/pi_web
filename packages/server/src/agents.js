...
const normalizeAgentName = (value) => {
  ...
  if (!normalized || !AGENT_NAME_PATTERN.test(normalized)) {
    throw createAgentError('INVALID_AGENT_NAME', 'Agent 名称只允许小写字母、数字和中划线');
  }
...
const ensureSupportedFields = (frontmatter) => {
  for (const key of Object.keys(frontmatter)) {
    if (!SUPPORTED_FRONTMATTER_FIELDS.has(key)) {
      throw createAgentError('UNKNOWN_AGENT_FIELD', `不支持的 agent 字段: ${key}`);
    }
  }
};
...
const parseAgentFile = (rawContent, filePath, sourceScope) => {
  ...
  try {
    ...
    throw createAgentError('INVALID_AGENT_FRONTMATTER', error?.message || `无法解析 ${filePath}`);
  }

  const frontmatter = parsed?.frontmatter && typeof parsed.frontmatter === 'object' ? parsed.frontmatter : {};
  const systemPrompt = typeof parsed?.body === 'string' ? parsed.body.trim() : rawContent.trim();
  ...
  if (normalizeString(frontmatter.name) && resolvedName !== normalizeAgentName(fileStem)) {
    throw createAgentError('INVALID_AGENT_NAME', `agent 文件名与 frontmatter.name 不一致: ${filePath}`);
  }
  ...
  const thinking = frontmatter.thinking === undefined ? undefined : normalizeThinkingLevel(frontmatter.thinking);
  if (frontmatter.thinking !== undefined && !thinking) {
    throw createAgentError('INVALID_AGENT_THINKING', `agent 的 thinking 配置非法: ${filePath}`);
  }

  const steps = frontmatter.steps === undefined ? undefined : normalizeInteger(frontmatter.steps);
  if (frontmatter.steps !== undefined && steps === undefined) {
    throw createAgentError('INVALID_AGENT_STEPS', `agent 的 steps 必须是大于等于 1 的整数: ${filePath}`);
  }
  ...
  try {
    ...
    throw createAgentError('INVALID_AGENT_PERMISSION_SCHEMA', error?.message || `agent 的 permission 配置非法: ${filePath}`);
  }
...
const normalizeAgentPayload = (name, payload, { requireScope = false, existing = null } = {}) => {
  ...
  for (const key of Object.keys(payload)) {
    if (!topLevelKeys.has(key)) {
      throw createAgentError('UNKNOWN_AGENT_FIELD', `不支持的 agent 配置字段: ${key}`);
    }
  }

  const normalizedName = normalizeAgentName(payload.name || name);
  if (payload.name && normalizedName !== normalizeAgentName(name)) {
    throw createAgentError('INVALID_AGENT_NAME', '请求体中的 name 必须与路径参数一致');
  }

  const description = normalizeString(payload.description ?? existing?.description);
  if (!description) {
  ...
  const mode = payload.mode === undefined ? (existing?.mode || 'all') : normalizeMode(payload.mode);
  if (!mode) {
    throw createAgentError('INVALID_AGENT_MODE', 'agent mode 只允许 primary、task、all');
  }

  const thinking = payload.thinking === undefined
    ? existing?.thinking
    : (payload.thinking === null ? undefined : normalizeThinkingLevel(payload.thinking));
  if (payload.thinking !== undefined && payload.thinking !== null && !thinking) {
    throw createAgentError('INVALID_AGENT_THINKING', 'agent thinking 配置非法');
  }

  const steps = payload.steps === undefined
    ? existing?.steps
    : normalizeInteger(payload.steps);
  if (payload.steps !== undefined && steps === undefined) {
    throw createAgentError('INVALID_AGENT_STEPS', 'agent steps 必须是大于等于 1 的整数');
  }
  ...
  try {
    ...
    throw createAgentError('INVALID_AGENT_PERMISSION_SCHEMA', error?.message || 'agent permission 配置非法');
  }

  const scope = payload.scope ?? existing?.sourceScope;
  if (requireScope && scope !== 'user' && scope !== 'project') {
    throw createAgentError('AGENT_SCOPE_REQUIRED', '创建 agent 时必须指定 scope');
  }

  const normalized = {
    name: normalizedName,
    description,
    display_name: normalizeString(payload.display_name ?? existing?.displayName) || undefined,
    mode,
...
const resolveScopeDir = async (cwd, scope) => {
  ...
  throw createAgentError('AGENT_SCOPE_REQUIRED', 'scope 必须是 user 或 project');
};
...
export async function saveAgent(cwd, name, payload, options = {}) {
  ...
  if (options.allowCreate === true) {
    try {
      ...
      throw createAgentError('AGENT_NAME_CONFLICT', `agent 已存在: ${normalized.name}`, 409);
    } catch (error) {
      if (error?.code !== 'ENOENT') {
