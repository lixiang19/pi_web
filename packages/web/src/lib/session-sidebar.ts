...
const buildSessionTree = (sessions: SessionSummary[], pinnedIds: Set<string>) => {
  ...
  const sortNodes = (nodes: SessionTreeNode[]) => {
    nodes.sort((left, right) => compareSessionsByPinnedAndTime(left.session, right.session, pinnedIds))
...
const filterTreeByQuery = (nodes: SessionTreeNode[], normalizedQuery: string): SessionTreeNode[] => {
  if (!normalizedQuery) {
    return nodes
  }
...
export const buildSessionProjects = (options: {
  ...
  return [...projectsById.values()]
    .map((project) => {
      const projectMatchesQuery = !normalizedQuery
        || `${project.label} ${project.projectRoot}`.toLowerCase().includes(normalizedQuery)
      const activeSessions = project.sessions.filter((session) => !session.archived)
      const archivedSessions = project.sessions.filter((session) => session.archived)
      const groups: SessionGroupView[] = []

      const rootSessions = activeSessions.filter((session) => session.worktreeRoot === session.projectRoot)
      if (rootSessions.length > 0) {
        const fullTree = buildSessionTree(rootSessions, pinnedIdSet)
        const groupMatchesQuery = !normalizedQuery
          || `project root ${rootSessions[0]?.branch || ''} ${project.projectRoot}`.toLowerCase().includes(normalizedQuery)
        const tree = projectMatchesQuery || groupMatchesQuery
          ? fullTree
        ...
        if (tree.length > 0) {
          groups.push({
            ...
            tree,
            lastUpdatedAt: Math.max(...rootSessions.map((session) => session.updatedAt)),
          })
        }
      }
      ...
      const worktreeGroups = [...worktreeMap.entries()]
        .map<SessionGroupView | null>(([worktreeRoot, sessions]) => {
          ...
          const groupMatchesQuery = !normalizedQuery
            || `${sessions[0]?.worktreeLabel || ''} ${sessions[0]?.branch || ''} ${worktreeRoot}`.toLowerCase().includes(normalizedQuery)
          const tree = projectMatchesQuery || groupMatchesQuery
            ? fullTree
            : filterTreeByQuery(fullTree, normalizedQuery)
          if (tree.length === 0) {
          ...
          return {
            ...
            label: sessions[0]?.worktreeLabel || relativeLabel(worktreeRoot, options.workspaceDir),
            branch: sessions[0]?.branch,
            worktreeRoot,
            sessions,
            tree,
            lastUpdatedAt: Math.max(...sessions.map((session) => session.updatedAt)),
          }
        })
      ...
      if (archivedSessions.length > 0) {
        ...
        const groupMatchesQuery = !normalizedQuery || 'archived'.includes(normalizedQuery)
        const tree = projectMatchesQuery || groupMatchesQuery
          ? fullTree
          : filterTreeByQuery(fullTree, normalizedQuery)
        if (tree.length > 0) {
          groups.push({
            ...
            tree,
            lastUpdatedAt: Math.max(...archivedSessions.map((session) => session.updatedAt)),
          })
        }
      }
