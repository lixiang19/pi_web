...
const isDirectoryExpanded = (directoryPath: string) => expandedDirectories.value.includes(directoryPath)

const isDirectoryLoading = (directoryPath: string) => loadingDirectories.value.includes(directoryPath)

const setDirectoryEntries = (directoryPath: string, entries: FileTreeEntry[]) => {
...
const loadDirectory = async (directoryPath: string, options?: { force?: boolean }) => {
  ...
  try {
    const payload = await getFileTree(normalizedDirectory, rootPath.value || normalizedDirectory)
    setDirectoryEntries(normalizedDirectory, payload.entries)
  } catch (caughtError) {
    fileTreeError.value = caughtError instanceof Error ? caughtError.message : String(caughtError)
  } finally {
    loadingDirectories.value = loadingDirectories.value.filter((path) => path !== normalizedDirectory)
  }
}
  ...
  <div class="flex h-full flex-col overflow-hidden rounded-[28px] border border-white/10 bg-black/35 backdrop-blur">
    <div class="border-b border-white/10 px-4 py-4">
      <div class="flex items-center justify-between gap-3">
        ...
        <Button variant="ghost" size="icon" class="rounded-full text-stone-400 hover:text-stone-100" @click="refreshTree">
          <RefreshCw class="size-4" :class="isDirectoryLoading(rootPath) ? 'animate-spin' : ''" />
        </Button>
      </div>
      <p class="mt-3 break-all rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2 font-mono text-[11px] leading-5 text-stone-400">
        {{ rootPath || '没有可展示的目录' }}
      </p>
    </div>
    ...
    <ScrollArea class="flex-1 px-2 py-3">
      ...
      <div v-else-if="visibleNodes.length === 0 && isDirectoryLoading(rootPath)" class="flex items-center gap-3 px-3 py-8 text-sm text-stone-500">
        <LoaderCircle class="size-4 animate-spin text-amber-300" />
        正在加载文件树...
      </div>

      <div v-else-if="fileTreeError" class="px-3 py-6 text-sm leading-6 text-red-200">
        {{ fileTreeError }}
      </div>

      <div v-else-if="visibleNodes.length === 0" class="px-3 py-8 text-sm text-stone-500">
        当前目录为空。
      </div>
      ...
      <div v-else class="space-y-1 pb-4">
        ...
        >
          <component
            :is="node.entry.kind === 'directory' ? (isDirectoryExpanded(node.entry.path) ? ChevronDown : ChevronRight) : ChevronRight"
            class="size-4 shrink-0"
            :class="node.entry.kind === 'directory' ? 'text-stone-500' : 'text-transparent'"
          />

          <component
            :is="node.entry.kind === 'directory' ? (isDirectoryExpanded(node.entry.path) ? FolderOpen : Folder) : FileCode2"
            class="size-4 shrink-0"
            :class="node.entry.kind === 'directory' ? 'text-amber-200' : 'text-stone-400'"
          />

          <span class="min-w-0 flex-1 truncate text-sm" :class="node.entry.kind === 'directory' ? 'text-stone-100' : 'text-stone-300'">
            {{ node.entry.name }}
          </span>

          <LoaderCircle
            v-if="node.entry.kind === 'directory' && isDirectoryLoading(node.entry.path)"
