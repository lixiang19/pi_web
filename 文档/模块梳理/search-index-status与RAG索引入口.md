...
Response.rag: { pending, indexed, indexFailed, latestIndexedAt, failedTargets }
```

诊断入口读取 `search_index_status` 和未处理 `rag.index_failed` 通知，返回失败目标、错误原因、更新时间和关联通知 ID。UI 从搜索页空查询态展示这些失败项，并复用手动刷新端点重建索引。
...
- `packages/server/src/routes/workspace-search.ts` — `/api/workspace/knowledge/diagnostics` 知识诊断端点
...
- **2026-05-16**：完成 V2 阶段 5 知识诊断入口
  - 搜索页可见 RAG 队列、最近索引时间和失败目标
  - 失败目标带错误原因和关联通知，可直接手动刷新
