...
- 2026-05-08 起，VPS 个人部署新增单用户固定密码认证：`/api/auth/session|login|logout` 公开，其余 `/api/*` 与终端 WebSocket 必须通过服务端 Session Cookie 鉴权。
...
- 负责提供 Web 私有部署安全边界：固定密码登录、内存 Session、`HttpOnly` Cookie、登录限流、API 全局鉴权和终端 WebSocket upgrade 鉴权。
