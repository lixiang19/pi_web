...
为 Pi Agent 会话提供显式转换工具，让 Agent 可调用 ridge Python Converter，将工作空间文件或 HTTPS URL 转为 Markdown。工具只做调用与结果返回，不在 Node 内自研 PDF/图片/音频解析，也不替代文件处理队列的落盘、归档、状态机职责。
...
- `packages/server/src/agent-permissions.ts`
  - `convert_file_to_markdown` / `convert_url_to_markdown` 映射为 `read` 权限。
  - `read: deny` 时移除转换工具。
...
| `convert_url_to_markdown` | 转换 HTTPS URL 网页 | `url` 只接受 HTTPS，实际 SSRF/私网拦截仍由 Python Converter 执行 | Markdown 正文 + job/artifact details |
...
- URL 工具默认 `task=document.markdown`、`options.engine=markitdown`、`mimeType=text/html`。
