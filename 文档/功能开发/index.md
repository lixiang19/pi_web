...
- **76 Exa 官方 API 网页内容工具**：`convert_url_to_markdown` 已从 Pi 转换工具中移除；URL 网页正文提取改为 `exa_get_contents`，由 `packages/server/src/exa-tools.ts` 直接调用 Exa 官方 `POST /contents` API。普通 Pi 会话和内部 `fleeting-agent` 都注册该工具；闪念 prompt 明确要求 URL 闪念优先调用 `exa_get_contents` 后写入剪藏或资料 Markdown。工具读取 `.env` / 环境变量 `EXA_API_KEY`，拒绝非 HTTP/HTTPS、localhost 和私网 URL，权限归类为 `read`。`exa-tools.test.ts` 覆盖请求体、鉴权配置、URL 边界和权限收口。已标 ✅。
- **77 运行时配置环境变量化**：新增 `packages/server/src/env.ts`，server 启动时加载仓库根目录到运行目录的 `.env` / `.env.local`，shell 环境变量优先级最高。Exa、Python Converter client、SiliconFlow embedding 配置统一改为环境变量读取，并提交 `.env.example`。`env.test.ts`、`conversion-service-client.test.ts`、`exa-tools.test.ts`、`siliconflow-embedding-client.test.ts` 覆盖配置读取。已标 ✅。
