...
3) **闪念临时附件**：`.ridge/fleeting-attachments` 目录模板已在 `workspace-chat.ts` 就位；本次任务 13 补齐附件上传 API、DB 引用字段、清理/迁移逻辑。会话附件 `session_attachments` 是另一套独立体系。

## 2026-05-11 任务 12 闪念数据模型与 Web 入口

### 实现要点

- 数据库迁移版本 8：`fleeting_notes` 增加 `type`（默认 'text'）、`suggestion`；修正 `status` 默认 'pending'、`analysis_status` 默认 'unanalyzed'；`UPDATE` 修复旧数据主状态（把旧 'active' 改 'pending'，旧 'pending' 改 'unanalyzed'）。
- `RIDGE_DB_SCHEMA_VERSION` 提升到 8；`RIDGE_DB_BOOTSTRAP_SQL`（v1）已包含正确字段定义（`type`/`suggestion`、status='pending'、analysis_status='unanalyzed'）；`CORE_TABLE_COLUMNS` 同步缺列修复定义；v8 只做旧数据 UPDATE 修复（把旧 'active' 改 'pending'，旧 'pending' 改 'unanalyzed'）。
- 后端路由：新增 `PATCH /api/workspace/fleeting/:id`，支持 content/status/analysisStatus/suggestion，strict 枚举校验。
- 创建 schema 用 `.strict()` + `refine`，拒绝 `projectId` / `project` 等字段传入，400 返回明确错误。
- `DELETE` 删除后调用 `cleanupFleetingAttachments`，安全检查路径不越界（`path.basename(noteId)` 过滤路径分隔符 + `fs.realpath(baseDir)` 解析真实路径 + `path.relative(baseReal, targetReal)` 检测 `..` 越界 + `targetReal.startsWith(baseReal + path.sep)` 兜底），不抛异常阻塞删除。
- process journal/clip 成功后也触发附件清理；task 未接入保留原闪念。
- 前端 API 全部切到 `/api/workspace/fleeting`；新增 `patchFleetingNote`；`FleetingNote` 类型增加 `type: string` 和 `suggestion: string | null`，`analysisStatus` 扩展 `failed`。
- `useInbox`：`hasPendingAnalysis` 改为只检测 `unanalyzed || analyzing`；`failed` 和 `suggested` 不再轮询；搜索范围加入 `suggestion` 字段。
- `InboxView.vue`：主标题从“收件箱”改为“闪念”；增加 Web 附件上传入口（真实 file input + 选择后展示文件名和大小，提示“暂未接入正式附件系统”）；固定处理按钮不再被 `analysisStatus` 禁用；`failed` 状态用红色背景展示“分析失败”。
- `index.ts` 同时挂载 `/api/fleeting`（兼容）和 `/api/workspace/fleeting`（spec）。

### 测试覆盖（未执行）

- `fleeting-api.test.ts`: 新增 strict payload 拒绝、PATCH 状态/分析状态/失败、枚举校验、DELETE 附件清理、安全边界测试。
- `useInbox.test.ts`: 新增 `failed` 不轮询、`suggestion` 搜索覆盖。
- `InboxView.test.ts`: 新增附件入口、固定按钮始终可用、`failed` 展示、标题改为“闪念”。
...
- **本次因 worktree 暂无法运行自动化测试，按用户明确指示不执行 npm/pnpm/vitest/playwright 命令；测试代码已新增/更新，待环境恢复后补跑。**

### 教训

- 版本化迁移的 bootstrap SQL、migration SQL、repair columns 三处必须同步；一处遗漏会导致新库/旧库行为不一致。
- `z.object().strict().refine()` 是拒绝未声明字段的可靠方式；比 `.passthrough()` 更安全。
- 删除时的附件清理必须做多层路径边界校验（`path.basename(noteId)` 过滤分隔符 → `fs.realpath(baseDir)` 解析真实路径 → `path.relative(baseReal, targetReal)` 检测 `..` 越界 → `startsWith(baseReal + path.sep)` 兜底），否则恶意 noteId 可能导致目录遍历删除。
- 轮询状态机要明确限定轮询范围；`failed` 如果参与轮询会造成无效请求风暴。
- UI 文案必须与设计对齐：统一叫“闪念”而不是“收件箱”。

## 2026-05-11 任务 13 闪念临时附件生命周期

### 实现要点

- 数据库迁移版本 9：新增 `fleeting_attachments` 表，字段包括 `attachment_id`、`note_id`、`original_name`、`stored_name`、`temp_path`、`final_path`、`mime_type`、`size`、`sha256`、`created_at`；索引 `idx_fleeting_attachments_note`；`FOREIGN KEY(note_id) REFERENCES fleeting_notes(note_id) ON DELETE CASCADE`。
- `RIDGE_DB_SCHEMA_VERSION` 提升到 9；`RIDGE_DB_BOOTSTRAP_SQL` 同步新增 `fleeting_attachments` 表定义；v9 migration SQL 为旧库补建表；`CORE_TABLE_COLUMNS` 同步缺列修复定义。
- 后端路由：
  - `POST /api/workspace/fleeting` 同时支持 JSON 和 multipart/form-data；`content` 可为空但必须有附件；`upload.fields([{ name: "attachments", maxCount: 20 }, { name: "files", maxCount: 20 }])` 处理文件上传，兼容 `attachments` 和 `files` 两个字段。
  - 附件保存到 `.ridge/fleeting-attachments/<noteId>/`，DB 记录引用；返回 note 包含 `attachments` 数组。
  - `GET /api/workspace/fleeting` 批量查询附件并按 noteId 分组返回。
  - `DELETE` 删除 note 后同时删除附件 DB 记录和临时目录；`cleanupFleetingAttachments` 复用路径安全校验。
  - `process/journal`：先写日记，再 `migrateAttachmentsToFinal`，最后删除 note + 清理临时目录。
  - `process/clip`：先插 clip，再 `migrateAttachmentsToFinal`，最后删除 note + 清理临时目录。
  - `process/task`：未接入，保留原闪念和临时附件。
  - `migrateAttachmentsToFinal` 使用 `fs.copyFile` 迁移到 `附件/`，保留原文件名；冲突时 `fs.access` 检测并追加短 ID（`base-${shortId}${ext}`）。
- 前端：
  - `createFleetingNote(content, files?)` 支持 `File[]`；`request` 自动识别 FormData。
  - `InboxView.vue`：`pendingAttachments` 从元数据数组改为 `File[]` 对象数组；支持移除单个附件；保存按钮允许无文字但有附件；保存成功清空附件，失败保留。
  - 列表展示附件数量/名称。
- 路径安全：
  - `sanitizeFileName` 取 `path.basename` 并替换非法字符；空名 fallback 为 `"unnamed"`。
  - `getSafeNoteAttachmentDir` 使用 `path.basename(noteId)` + `fs.realpath(baseDir)` + `path.relative` 检测 `..` + `startsWith(baseReal + path.sep)` 兜底。
  - 正式附件目录同样使用 `realpath` + `relative` 校验。
- 不写 `search_index_status`；临时附件不进 RAG、不进图谱、不允许 workspace MCP 读取。
...
- `fleeting-api.test.ts`（29 tests）：
  ...
  - journal 成功迁移附件到 `附件/` 并清理临时目录；clip 成功迁移附件。
  - 附件名称冲突循环去重直到唯一。
  - process 失败（clip schema 校验失败）保留原闪念和临时附件；task process 保留原闪念。
  - 迁移中途失败：真实制造 copyFile 失败（篡改 temp_path），验证已复制的正式附件被删除、DB `final_path` 重置为 null、note 保留、临时目录保留。
  - 不写 `search_index_status`；路径遍历文件名安全化（含 Windows 反斜杠）。
- `ridge-db-migration.test.ts`：更新断言包含 `fleeting_attachments` 表。
- `InboxView.test.ts`：新增带附件保存调用、附件选择后保存、保存失败保留附件、列表展示附件数量/名称、纯附件无文字可保存；修复 Vue unmount 内部错误。
- `useInbox.test.ts`：新增 `captureNote(text, files)` 覆盖、默认空附件数组。
- 后端测试：fleeting-api 专项 29 项全部通过。`packages/server` 全量跑时 `session-attachments` 存在历史 `database is locked` 问题、`file-tree-http` 存在历史 401 问题，与本次改动无关。
- 前端测试：184 passed / 184（web 全量，0 errors，已修复 Vue unmount 内部错误）。
- `npm run check`：eslint 0 errors（剩余 16 warnings 均为历史 `any` 类型），vue-tsc 0 errors。

### 教训

- 版本化迁移的 bootstrap SQL、migration SQL、repair columns 三处必须同步；本次 `RIDGE_DB_BOOTSTRAP_SQL` 中 `search_index_status` 表名被意外截断，导致所有用到 `RIDGE_DB_BOOTSTRAP_SQL` 的测试崩溃；修复后需逐行核对 SQL 完整性。
- `z.object({ content: z.string().optional().default("") }).strict()` 对 multipart 请求（`req.body` 为 `{}`）会校验通过，因为 `default` 在 `strict` 之前生效；但 `refine` 会基于解析后的值判断，不能放在 `strict` 之后（`strict` 只检查键，不检查值）。更好的做法是在 handler 中手动判断 `content.trim() && files.length === 0` 抛出 400，而不是依赖 zod refine。
- `migrateAttachmentsToFinal` 必须是 `async` 并 `await`，因为它内部有 `fs.access` 和 `fs.copyFile`；如果在同步 transaction 中调用，会丢失异步上下文并在 db 关闭后执行 `update.run()`，导致 `database is not open` 错误。
- Express 中间件顺序：`upload.fields([{ name: "attachments" }, { name: "files" }])` 必须在 `createFleetingRouter` 之前；对无文件请求，multer 会把 `req.body` 解析为 `{}`，所以 `req.body.content` 可能是 `undefined`，需要在 handler 中兼容。
- 前端测试中 `File` 对象在 jsdom 中可用，但 `Object.defineProperty(inputEl, "files", { value: [file] })` 才能触发 change 事件的 files；`wrapper.trigger("change", { files: [file] })` 会被忽略。
- 前端 `FleetingNote` 接口增加 `attachments` 字段后，所有 mock 数据必须同步增加，否则 `vue-tsc` 报错；可选字段 `attachments?` 能减少破坏面，但 spec 要求返回时必含，因此选择必含并在测试中补齐。
