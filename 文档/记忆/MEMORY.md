...
- Migration v13：devices 表增加 `token` 字段（由 `CORE_TABLE_COLUMNS` repair 机制处理，不在 migration SQL 中 `ALTER TABLE` 避免 duplicate column）。
- Migration v13：projects 表 `device_id` 空字符串清理为 NULL；新增 `idx_projects_device_path` UNIQUE INDEX 实现同设备同路径唯一。
