...
- **双工作模式**：代码模式专注工程开发，助手模式专注知识对话，一键切换
- **移动端支持**：触控原生设计，碎片场景适配，语音优先输入
...
| 特征 | 说明 | 实现参考 |
|------|------|----------|
| 极简干净 | 没有多余装饰，纯粹的留白 | 移除左侧指示条、边框等视觉噪音 |
| 柔和背景色 | hover 和选中状态都是很淡的灰色 | 选中：`bg-[hsl(0_0%_96%)]`，Hover：`hover:bg-[hsl(0_0%_98%)]` |
| 无左侧指示条 | 不像传统侧边栏那样有彩色边条标记当前项 | 完全移除 `border-left` 或 `before` 指示器 |
| 文字颜色统一 | 选中时文字颜色变化很小，主要靠轻微背景区分 | 统一使用 `text-foreground`，不使用 `text-accent-foreground` |
| 克制不干扰 | 界面元素不抢夺用户注意力 | 避免高饱和度颜色作为选中背景 |
# 文档/功能开发
